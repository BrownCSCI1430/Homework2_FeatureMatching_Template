...
Let's look at the Fourier transform demo from class. The Fourier transform lets us operate on images with respect to their frequency content via the \textbf{amplitude} (or magnitude) and the \textbf{phase}. From your uv environment, run these commands:
\begin{verbatim}
...
\begin{itemize}
    ...
    \item ``DC only'' means effect is only on zero-frequency component (DC offset).
    \item ``Scales same'' means shrinks when the input shrinks; ``Scales inverse'' means expands when input shrinks.
\end{itemize}
\end{question}
...
\textbf{Input transform} & \rotatebox{90}{\textbf{Invariant}} & \rotatebox{90}{\textbf{Intensity increases}} & \rotatebox{90}{\textbf{DC only}} & \rotatebox{90}{\textbf{2D translates}} & \rotatebox{90}{\textbf{2D rotates}} & \rotatebox{90}{\textbf{2D scales same}} & \rotatebox{90}{\textbf{2D scales inverse}} \\
\hline
\textbf{Change intensity} ($I += 0.2$) & & & & & & & \\
...
To detect these features, the algorithm approximates the strength of any change in the windowed auto-correlation function $E(u, v)$ over shifts $u,v$. We look at the second derivative term in the Taylor series expansion of $E$ to consider the local shape of $E(u, v)$ via the \emph{structure tensor} $M$ (often also called the second moment matrix).
...
where $w$ is typically a Gaussian window function. Then, we analyze $M$ to determine if that pixel is a corner or not.

\textit{Note:} This question requires careful study and a precise answer.
\end{question}

\begin{subquestion}[points=3]
Consider the spatial derivatives $I_x$ and $I_y$ at each pixel in the window $W$. Suppose we plotted these $(I_x, I_y)$ values as points in a 2D scatter plot. Describe the shape or distribution of the points that would form for (i) a flat region, (ii) an edge, and (iii) a corner. \textbf{[3--5 sentences]}
\end{subquestion}
...
\begin{subquestion}[points=3]
Suppose we consider the eigendecomposition of the structure tensor $M$. How do the eigenvalues of $M$ relate to the local image intensity patterns, and how might we interpret the eigenvalues geometrically as an ellipse? \textbf{[6--8 sentences]}
\end{subquestion}
...
$M$ is constructed using image derivatives $I_x$ and $I_y$ from the horizontal and vertical axes. We know that real-world edges are rarely axis-aligned, yet the Harris detector is rotationally invariant. Mathematically, why does Harris' cornerness response function $$R=det(M)-\alpha(trace(M))^2$$ have this rotation invariance property \textbf{without} needing to perform a factorization of $M$, i.e., via eigendecomposition to find the principal directions (eigenvectors)?
\end{subquestion}

\begin{answer}[height=24]
TODO: Your answer here.
\end{answer}
